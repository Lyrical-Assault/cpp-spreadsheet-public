...
public:
    ...
    virtual double Evaluate(std::function<double(Position pos)> func) const = 0;
...
public:
    ...
    double Evaluate(std::function<double(Position pos)> func) const override {
        double res = 0.0;
        if(type_ == Type::Add) {
            res = lhs_->Evaluate(func) + rhs_->Evaluate(func);
...
double FormulaAST::Execute(std::function<double(Position pos)> func) const {
